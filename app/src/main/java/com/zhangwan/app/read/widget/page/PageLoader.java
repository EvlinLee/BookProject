...
import com.zhangwan.app.read.model.bean.BookRecordBean;
...
import com.zhangwan.app.read.utils.IOUtils;
...
public abstract class PageLoader {
    ...
    private boolean isPrvChapter = false;
     ...
     * 打开指定章节
     */
    public void openChapter(boolean isPrvChapter) {
        this.isPrvChapter = isPrvChapter;
        ...
        if (parseCurChapter()) {
            ...
            if (!isChapterOpen) {
                ...
                if (position == 0) {
                    isFirstPage = true;
                    isLastPage = false;
                } else if (position + 1 >= mCurPageList.size()) {
                    isFirstPage = false;
                    isLastPage = true;
                }
    ...
    boolean prev() {
        ...
        if (mStatus == STATUS_FINISH) {
            ...
            if (prevPage != null) {
                ...
                isLastPage = false;
                if (mCurPage.position == 0 && hasPrevChapter()) {
                    isFirstPage = true;
                }
                return true;
            }
        ...
        if (!hasPrevChapter()) { // 没有上一章
            return false;
        }
//
//        mCancelPage = mCurPage;
//        if (parsePrevChapter()) { // 获取上一章最后一页
//            mCurPage = getPrevLastPage();
//        } else {
//            mCurPage = new TxtPage();
//        }
    ...
    boolean next() {
        ...
        if (mStatus == STATUS_FINISH) {
            ...
            if (nextPage != null) {
                ...
                mPageView.drawNextPage();
                isFirstPage = false;
                if (mCurPage.position + 1 == mCurPageList.size() && hasNextChapter()) {
                    isLastPage = true;
                }
                return true;
            }
...
//        preLoadNextChapter();
        ...
        try {
            ...
            if (mCurPageList != null) {
                if (mCurPageList.isEmpty()) {
                    Log.d("PageLoader", "dealLoadPageList: 1");
                    ...
                    Log.d("PageLoader", "dealLoadPageList: 2");
                    mStatus = STATUS_FINISH;
                }
            } else {
                Log.d("PageLoader", "dealLoadPageList: 3");
                mStatus = STATUS_LOADING;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /********************************* 强大的分割线 ************************************/
    public boolean getIsFirstPage() {  // 当前页是否为当前章节的第一页
        return isFirstPage;
    }

    public boolean getIsLastPage() {  // 当前页是否为当前章节的最后一页
        return isLastPage;
    }
}
