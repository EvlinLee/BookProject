...
import com.zhangwan.app.db.BookCatalogue;
import com.zhangwan.app.db.BookList;
...
//    private List<BookCatalogue> directoryList = new ArrayList<>();
    ...
    //    private NewBookUtil mBookUtil;
    private PageEvent mPageEvent;
    private TRPage currentPage;//当前页
...
//                    mContext.getResources(), R.drawable.main_bg, mWidth, mHeight));
            ...
            setBgBitmap(bitmap);
            //设置字体颜色
            setM_textColor(Color.rgb(81, 83, 83));
            setBookPageBg(Color.rgb(40, 40, 40));
        } else {
            //设置背景
...
//                word.append(strLine);
        ...
        //画章
        int nChaterWidth = (int) mBatterryPaint.measureText(bookChatper) + 1;
        c.drawText(bookChatper, mWidth - marginWidth - nChaterWidth, statusMarginBottom + mBatterryFontSize, mBatterryPaint);
...
//                Toast.makeText(mContext, "当前是第一页", Toast.LENGTH_SHORT).show();
                TRPage prevLastPage = getPrevLastPage();
                cancelPage = prevLastPage;
                onDraw(mBookPageWidget.getCurPage(), prevLastPage.getLines(), true);
                prevLastPage = getPrePage();
                onDraw(mBookPageWidget.getNextPage(), prevLastPage.getLines(), true);
                currentPage = prevLastPage;
            }
            m_isfirstPage = true;
            return;
        } else {
...
//        m_mbBufLen = 0;
    ...
    public Boolean cacheBookByNetwork(ChatperDetailBean result) {
        bookName = result.getBookName();
        bookChatper = result.getTitle();
        return mBookUtil.cacheBookByNetwork(result);
    }
...
//                m_mbBufLen = mBookUtil.getBookLen();
    ...
    public void init() {
        ...
        PageFactory.mStatus = Status.FINISH;
    ...
    public void initPre() {
        ...
        currentPage = getPageForBegin(mBookUtil.getBookLen());
        drawStatus(mBookPageWidget.getCurPage());
        if (mBookPageWidget != null) {
            currentPage(true);
        }
     ...
     * @return:获取上一个章节的最后一页
     */
    private TRPage getPrevLastPage() {

        long end = currentPage.getEnd();

        mBookUtil.setPostition(end - 1);
        currentPage.setLines(getNextLines());
        currentPage.setEnd(mBookUtil.getPosition());
        return currentPage;
