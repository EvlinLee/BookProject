...
import com.zhangwan.app.bean.Cache;
...
//        int cachePos = (int) (position / cachedSize);
    ...
    //缓存网络获取的章节  todo
    public boolean cacheBookByNetwork(ChatperDetailBean bean){
        ...
        while (true){
            int tempSize = sb.length() > cachedSize ? cachedSize : sb.length();
            if (bookLen == sb.length()){
                break;
            }
            char[] buf = new char[tempSize];
            sb.getChars((int) bookLen, tempSize, buf, 0);
